...
def intersection_sparse(sparse_matrix, rows_list_original, cols_list_original):
    ...
    # modified row and column list to create intersection matrix
    rows_list, cols_list = [], []
    ...
    data_list = [sparse_matrix[r, cols_list[i]]
...
def cur(sparse_matrix, no_cols, no_eigen_values, energy=1):
    ...
    def select_columns(sparse_matrix_csc, select_col=True):
        ...
        # col_prob contains (indices of column, probabilty of that column)
        col_prob = []
        ...
        for i in range(no_cols):
            C[:, i] = sparse_copy.getcol(
                col_prob[i][0])/math.sqrt(no_cols*col_prob[i][1])
    ...
    R, row_prob = select_columns(
        sparse_matrix.transpose().tocsc(), select_col=False)

    # create W matrix (intersection of C and R)
    W = intersection_sparse(sparse_matrix, sorted(
        [x[0] for x in row_prob]), sorted([x[0] for x in col_prob]))
    print('Building C, R, W matrix took ' +
          '{0:.2f}'.format(time.time() - start) + ' secs.')

    # perform svd on W
    x, z, yt = np.linalg.svd(W.todense(), full_matrices=False)
    # x, z, yt = svd_retain_energy(W, no_eigen_values, energy)

    # form U matrix
    U = np.dot(np.dot(np.transpose(yt), np.linalg.matrix_power(
        np.diag(np.reciprocal(z)), 2)), np.transpose(x))
